...
/** Returns a durable cloud/public URL. No external source is resolved in the browser. */
...
/** Dynamic/article music is always a static cloud-backed audio item. */
...
export const useMusicPlayer = create<MusicPlayerState>((set, get) => ({
  ...
  prepareTrack: (index) => {
    ...
    if (!track?.mp3url) {
      set({ switching: false, isLoading: false, audioError: true, audioErrorMessage: "该歌曲的 云端音频文件不可用。" });
      return null;
    }
    set({
      currentIndex: index,
