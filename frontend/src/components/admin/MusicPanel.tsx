  ...
  return (
    ...
    >
      ...
      <div className="space-y-4">
        <div>
          <label className="mb-1.5 block text-xs font-medium text-adm-text-secondary">云端音频文件</label>
          {audioUrl ? (
            <div className="flex items-center justify-between rounded-lg border border-adm-border bg-adm-input px-3 py-2.5"><div className="flex min-w-0 items-center gap-2"><Music className="h-4 w-4 shrink-0 text-adm-text-tertiary" /><span className="truncate text-sm text-adm-text">{audioName || name || "云端音频已上传"}</span></div><label className="cursor-pointer text-xs text-adm-text-secondary hover:text-adm-text">重新上传<input type="file" accept="audio/mpeg,audio/mp3,audio/wav,audio/ogg,audio/aac" className="hidden" onChange={(event) => { void handleUploadAudio(event.target.files); event.target.value = ""; }} /></label></div>
          ) : <label className="flex cursor-pointer flex-col items-center justify-center rounded-xl border border-dashed border-adm-border py-6 transition-colors hover:border-gray-400"><input type="file" accept="audio/mpeg,audio/mp3,audio/wav,audio/ogg,audio/aac" className="hidden" onChange={(event) => { void handleUploadAudio(event.target.files); event.target.value = ""; }} />{uploadingAudio ? <Loader2 className="h-8 w-8 animate-spin text-adm-text-tertiary" /> : <><Upload className="h-7 w-7 text-adm-text-tertiary" /><p className="mt-1.5 text-sm text-adm-text-tertiary">点击上传 云端音频文件</p><p className="mt-0.5 text-[11px] text-adm-text-tertiary">支持 MP3/WAV/OGG/AAC，最大 50MB</p></>}</label>}
        </div>
