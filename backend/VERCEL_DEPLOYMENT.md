...
Vercel 生产请求默认不会执行 DDL。首次部署请在本地、CI 或受信任的维护机器上，对目标数据库受控执行唯一的初始化命令：

```bash
cd backend
DB_HOST=... DB_USER=... DB_PASSWORD=... DB_NAME=... DB_SSL=true npm run db:init
```

`db:init` 只会创建缺失表、补齐当前支持的加字段兼容项，并创建缺失的站点设置、默认歌单和首个管理员；可以重复运行，不会删除数据或重置已有管理员密码。它不会创建数据库本身，须先在数据库服务商处创建 `DB_NAME` 并授予对应用户权限。

不要在 Vercel 函数请求、构建步骤或冷启动中运行初始化，也不要设置 `DB_SYNC_ON_BOOT=true`；如模型升级涉及业务数据迁移，请按该版本的发布说明单独执行经过审核的迁移。

R2 歌单历史迁移仅适用于从旧版插件升级。它会先生成 `backups/music-plugin-migration-*.json` 清单；审核并备份数据库后，才可执行：

```bash
npm run music:migrate-r2
npm run music:migrate-r2 -- --apply
```

`--apply` 会修改数据库并清理旧插件数据，不属于常规初始化流程。

## 四、前端配套修改

豆瓣快照字段已由 `db:init` 自动补齐，无需在首次部署时额外执行迁移。然后在 Vercel 环境变量中设置高强度 `CRON_SECRET`。`vercel.json` 中的**每日**定时任务会以 `Authorization: Bearer $CRON_SECRET` 对 `GET /api/douban/cron-sync` 发起请求；若当前 Vercel 套餐不支持 Cron，请使用外部调度器以同一认证头调用该 endpoint。首次上线后请在后台手动同步一次；公共影单请求只读取 TiDB 快照，绝不会实时抓取豆瓣。
...
**首次部署恢复方式**：该错误会使对应的 `CREATE TABLE` 语句整体失败，不会留下半张表。部署修复后的代码后，先受控执行 `npm run db:init`，再请求 `/api/health` 验证。**不要为这个首次建表错误删除已有业务表。**
